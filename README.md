...
| 类 | 文件 | 职责 |
|---|------|------|
| `SerialPort` | `inc/serial_port.h` / `src/serial_port.cpp` | 串口打开/关闭、带超时的同步读取 |
| `ImuParser` | `inc/imu_parser.h` / `src/imu_parser.cpp` | 字节流缓冲、帧同步、校验和验证、字段提取 |
| `ImuPublisher` | `inc/imu_publisher.h` / `src/imu_publisher.cpp` | ROS Publisher 管理、消息构建与缩放 |
| `ImuDriverNode` | `inc/imu_driver_node.h` / `src/imu_driver_node.cpp` | 参数读取、模块组合、主循环、诊断 |
...
| 话题 | 消息类型 | 发布条件 |
|------|----------|----------|
| `~/imu_serial` | `imu_ros_driver/ImuData` | `publish_custom=true` |
